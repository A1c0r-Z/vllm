...
class MambaMixer2(MambaBase, PluggableLayer):
    ...
    **selective** state spaces)
    ...
    def __init__(
        ...
        # ReplaySSM opt-in: cache recent SSM inputs in a per-layer ring buffer
        # (x_cache/dt_cache/B_cache) and replay them on decode instead of writing
        # the full recurrent state back to HBM each step.
        ...
        # KV-cache page: (conv_state, ssm_state) baseline, or the 5-tensor
        # (conv_state, ssm_state, x_cache, dt_cache, B_cache) ReplaySSM page.
        self.kv_cache = tuple(
    ...
    def conv_ssm_forward(
        ...
        if attn_metadata_raw is not None:
            ...
            if self.use_cache_kernel:
                if len(self.kv_cache) != 5:
                    raise ValueError(
                        "ReplaySSM Mamba2 decode requires the 5-tensor KV-cache "
        ...
        if has_decode:
            ...
            if self.use_cache_kernel:
                # ReplaySSM decode: recompute the output by replaying the input
                # ring buffer instead of writing the recurrent state back to HBM
                # every step (state is rebuilt + flushed once per window).
                if is_mamba_cache_all:
                ...
                if num_accepted_tokens is not None or query_start_loc_d is not None:
                    raise ValueError(
                        "--use-replayssm does not support speculative or varlen decode"
                    )
                if (
                    attn_metadata.write_pos_d is None
                    or attn_metadata.is_flush_d is None
                ):
                    raise ValueError(
                        "ReplaySSM decode requires write_pos/is_flush metadata"
                    )
                ...
                if self.cached_kernel_variant == "output_only":
                    if attn_metadata.bc_pre_scratch is None:
                        raise ValueError(
                            "ReplaySSM output_only route requires bc_pre scratch"
                        )
                    selective_state_update_replayssm_output_only(
                        ssm_state,
                        hidden_states_d,
